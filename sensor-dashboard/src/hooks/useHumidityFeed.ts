...
import { db, firebaseConfigured, type HumidityRecord } from '../firebase';

export type HumidityDoc = HumidityRecord & { id: string };

export function useHumidityFeed(
  maxItems: number = 200,
  timeRangeMinutes?: number,
  startDate?: Date,
  endDate?: Date
) {
  const [data, setData] = useState<HumidityDoc[]>([]);
  ...
  useEffect(() => {
    ...
    const ref = collection(db, 'humidity');
    ...
    const unsub = onSnapshot(
      q,
      (snap) => {
        const items: HumidityDoc[] = snap.docs
          .map((d) => {
            ...
            return { id: d.id, timestamp: ts, value } as HumidityDoc;
          })
          .filter(Boolean) as HumidityDoc[];
        let filtered = items;
        if (timeRangeMinutes) {
          const now = Date.now();
...
export default useHumidityFeed;
