      ...
      <table className="w-full border-collapse min-w-[320px]">
        <thead>
          <tr>
            ...
            <th className="text-left border-b border-gray-300 p-1.5 sm:p-2 text-xs sm:text-sm font-semibold text-gray-800">Ánh sáng (lx)</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r, idx) => (
            <tr key={r.id} className="hover:bg-gray-100 transition-colors duration-150">
              ...
              <td className="border-b border-gray-200 p-1.5 sm:p-2 text-xs sm:text-sm text-gray-700 font-medium">{Math.round(r.value)}</td>
            </tr>
          ))}
          {!rows.length && (
            <tr>
